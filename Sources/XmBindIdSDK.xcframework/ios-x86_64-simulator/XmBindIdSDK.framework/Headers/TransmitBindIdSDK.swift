...
/** XmBindIdLoginSpBoundType
 Available types for the login sp bound.
 */
public enum XmBindIdLoginSpBoundType : String, CaseIterable, Equatable, Hashable, Codable {
   /** Indicates that the login hint value is a user id */ 
   case sub
   /** Indicates that the login hint value is a user alias. If Alias type is used it is highly recommended to use an encrypted authentication request (see encrypted flag) to avoid the possibility of exposing sensitive data.*/ 
   case alias
} 
...
@objc(XmBindIdLoginSpBound)
public class XmBindIdLoginSpBound : NSObject, Codable { 

public var type: XmBindIdLoginSpBoundType!

public var value: String!

public init(type: XmBindIdLoginSpBoundType, value: String) {
     super.init()
     self.type = type
     self.value = value
}
...
/**
 Optional. Type and value for the login hint, which is used as a hint for the user’s login identifier (e.g., their email address)
 */
public var spBound: XmBindIdLoginSpBound?
}
